        ...
        for file in files:
            if file.endswith(".html"):
                compress_html(
                    ...
                    os.path.join(dest_dir, os.path.relpath(root, source_dir), file),
                )
...
if __name__ == "__main__":
    ...
    print(
        "Compress files from {} bytes to {} bytes (gain {:.2f}%)".format(
            src_size, dest_size, (src_size - dest_size) / src_size * 100
        )
    )

    # get git version
    git_version = os.popen("git describe --tags --always --dirty").read().strip()
