...
static int ble_adv_data_set(tuya_ble_service_params_s *dev_info, TKL_BLE_DATA_T *p_adv, TKL_BLE_DATA_T *p_scan_rsp)
{
    ...
    /* BLE link flag */
    segment_length = 1;
    p_adv->p_data[p_adv->length + segment_length] = 0x01;
    segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = 0x06;
    segment_length++;
    p_adv->p_data[p_adv->length] = segment_length - 1;
    p_adv->length += segment_length;
    /* Service UUID */
    segment_length = 1;
    p_adv->p_data[p_adv->length + segment_length] = 0x02;
    segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = 0x50;
    segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = 0xFD;
    segment_length++;
    p_adv->p_data[p_adv->length] = segment_length - 1;
    p_adv->length += segment_length;
    /* Service data */
    segment_length = 1;
    p_adv->p_data[p_adv->length + segment_length] = 0x16;
    segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = 0x50;
    segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = 0xFD;
    segment_length++;
    // Frame Control
    p_adv->p_data[p_adv->length + segment_length] = 0x43;
    segment_length++;
    p_adv->p_data[p_adv->length + segment_length] = 0x00;
    segment_length++;
    ...
    // DIY
    segment_length = 1;
    p_scan_rsp->p_data[p_scan_rsp->length + segment_length] = 0xFF;
    segment_length++;
    // company code: 0x07D0
    p_scan_rsp->p_data[p_scan_rsp->length + segment_length] = 0xD0;
    segment_length++;
    p_scan_rsp->p_data[p_scan_rsp->length + segment_length] = 0x07;
    segment_length++;
    // ENC mode
    ...
    p_scan_rsp->p_data[p_scan_rsp->length + segment_length] = 0x00;
    segment_length++;
    p_scan_rsp->p_data[p_scan_rsp->length + segment_length] = 0x0C;
    segment_length++;
    // flag
...
static int ble_recv_data_unpack(uint8_t *data, uint32_t len, tuya_ble_pack_s *pack)
{
    ...
    { // first pack
        ...
        pack->version = data[pack_offset] >> 4;
        pack_offset++;
    }
...
static int ble_recv_data_decrypt(tuya_ble_pack_s *recv_pack, tuya_ble_frame_plain_s *recv_frame)
{
    ...
    // check encrypt key
    if (0 == sg_ble_service_params->key_mask)
    { // no key, first pack
...
static OPERATE_RET ble_service_get_device_info(const uint8_t *srand, const uint8_t *register_key, uint8_t *p_data, uint16_t len)
{
    ...
    //* firmware version 1
    p_data[data_offset] = (FIREWARE_VERSION_1 >> 8) & 0xFF;
    data_offset++;
    p_data[data_offset] = FIREWARE_VERSION_1 & 0xFF;
    data_offset++;
    //* tuya ble protocol version
    p_data[data_offset] = (TUYA_BLE_PROTOCOL_VERSION >> 8) & 0xFF;
    data_offset++;
    p_data[data_offset] = TUYA_BLE_PROTOCOL_VERSION & 0xFF;
    data_offset++;
    //* flag
    p_data[data_offset] = 0x05;
    data_offset++;
    // bond
    p_data[data_offset] = 0x00;
    data_offset++;
    //* srand
    memcpy(&p_data[data_offset], srand, 6);
    data_offset += 6;
    //* hardware version 1
    p_data[data_offset] = (HARDWARE_VERSION_1 >> 8) & 0xFF;
    data_offset++;
    p_data[data_offset] = HARDWARE_VERSION_1 & 0xFF;
    data_offset++;
    //* register_key + 0(16 byte)
    memcpy(&p_data[data_offset], register_key, 16);
    data_offset += 32;
    //* fireware version 2
    p_data[data_offset] = (FIREWARE_VERSION_2 >> 16) & 0xFF;
    data_offset++;
    p_data[data_offset] = (FIREWARE_VERSION_2 >> 8) & 0xFF;
    data_offset++;
    p_data[data_offset] = FIREWARE_VERSION_2 & 0xFF;
    data_offset++;
    //* hardware version 2
    p_data[data_offset] = (HARDWARE_VERSION_2 >> 16) & 0xFF;
    data_offset++;
    p_data[data_offset] = (HARDWARE_VERSION_2 >> 8) & 0xFF;
    data_offset++;
    p_data[data_offset] = HARDWARE_VERSION_2 & 0xFF;
    data_offset++;
    //* communication type
    p_data[data_offset] = 0x00;
    data_offset++;
    p_data[data_offset] = 0x0C;
    data_offset++;
    //* flag2
    p_data[data_offset] = 0x00;
    data_offset++;
    ...
    p_data[data_offset] = (MCU_FIREWARE_VERSION >> 16) & 0xFF;
    data_offset++;
    p_data[data_offset] = (MCU_FIREWARE_VERSION >> 8) & 0xFF;
    data_offset++;
    p_data[data_offset] = MCU_FIREWARE_VERSION & 0xFF;
    data_offset++;
    //* mcu hardware version
    p_data[data_offset] = (MCU_HARDWARE_VERSION >> 16) & 0xFF;
    data_offset++;
    p_data[data_offset] = (MCU_HARDWARE_VERSION >> 8) & 0xFF;
    data_offset++;
    p_data[data_offset] = MCU_HARDWARE_VERSION & 0xFF;
    data_offset++;
    //* wifi flag
    p_data[data_offset] = 0x01;
    data_offset++;
    //* reserve
    data_offset++;
    //* device function
    ...
    data_offset++;
    //* packet max size len
    p_data[data_offset] = 0x02;
    data_offset++;
    p_data[data_offset] = APP_PACK_DATA_MAX >> 8;
    data_offset++;
    p_data[data_offset] = APP_PACK_DATA_MAX;
    data_offset++;
    //* sl len
    data_offset++;
...
static int ble_rsp_data_pack_and_send(uint8_t *frame, uint32_t frame_size)
{
    ...
    do
    {
        ...
        memcpy(&send_data[data_offset], &frame[frame_size - remain_len], copy_len);
...
int tuya_ble_service_start(tuya_ble_service_init_params_t *init_params, ble_token_get_callback cb)
{
    ...
    if (20 == strlen((const char *)sg_ble_service_params->uuid))
    {
