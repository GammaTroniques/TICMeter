...
===============================================================================*/
// clang-format off
#define LINKY_BUFFER_SIZE 32*1024 // The size of the UART buffer
...
uint8_t linky_buffer[LINKY_BUFFER_SIZE] = {0}; // The UART buffer

raw_group_t raw_groups[GROUP_COUNT] = {0}; // store raw data of each group
...
static uint8_t *linky_frame = NULL;          // The received frame from the linky
...
static uint8_t linky_read()
{
    ...
    do
    {
        linky_rx_bytes += uart_read_bytes(UART_NUM_1, linky_buffer + linky_rx_bytes, (LINKY_BUFFER_SIZE - 1) - linky_rx_bytes, 500 / portTICK_PERIOD_MS);
...
static char linky_decode()
{
    ...
    ESP_LOGI(TAG, "Found %ld fields, with %ld groups", linky_decode_count, current_group_index);
...
static void linky_create_debug_frame(uint8_t mode_std)
{
    ...
    if (mode_std == 2)
    {
        ...
        linky_rx_bytes = sizeof(linky_std_debug_buffer_bad);
        return;
    }
