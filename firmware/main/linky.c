    ...
    for (;;)
    {
        ...
        if (xQueueReceive(linky_uart_queue, (void *)&event, (TickType_t)portMAX_DELAY))
        {
            ...
            case UART_DATA:
                ...
                uart_read_bytes(UART_NUM_1, linky_buffer + linky_frame_size, to_read, 500 / portTICK_PERIOD_MS);
                linky_frame_size += to_read;
                ESP_LOGD(TAG, "Add %ld tot: %ld", to_read, linky_frame_size);
...
void linky_set_mode(linky_mode_t newMode)
{
    ...
    if (!uart_is_driver_installed(UART_NUM_1))
    {
        ...
        esp_err_t ret = uart_driver_install(UART_NUM_1, RX_BUF_SIZE, 0, 64, &linky_uart_queue, 0);
        if (ret != ESP_OK)
        {
        ...
        ret = uart_set_wakeup_threshold(UART_NUM_1, 1);
        if (ret != ESP_OK)
        {
            ESP_LOGE(TAG, "uart_set_wakeup_threshold failed: %s", esp_err_to_name(ret));
            return;
        }
        ret = esp_sleep_enable_uart_wakeup(UART_NUM_1);
        if (ret != ESP_OK)
        {
            ESP_LOGE(TAG, "esp_sleep_enable_uart_wakeup failed: %s", esp_err_to_name(ret));
            return;
        }
    }
    else
    {
        esp_err_t ret = uart_set_baudrate(UART_NUM_1, baud_rate);
        if (ret != ESP_OK)
        {
...
{
    uint32_t timeout = (xTaskGetTickCount() * portTICK_PERIOD_MS) + 7000; // 5 seconds timeout
    esp_err_t ret;
    ...
    bool hasFrame = false;
    size_t length = 0;
    ret = uart_get_buffered_data_len(UART_NUM_1, &length);
    if (ret != ESP_OK)
    {
        ESP_LOGE(TAG, "uart_get_buffered_data_len failed: %s", esp_err_to_name(ret));
    }

    ESP_LOGI(TAG, "Buffered data length: %zu", length);

    // uart_set_rx_timeout(UART_NUM_1, 102);
    do
    {
        // uint32_t free_space = (LINKY_BUFFER_SIZE - 1) - linky_rx_bytes;
#define TO_READ RX_BUF_SIZE
        // uint32_t to_read = (free_space > TO_READ) ? TO_READ : free_space;
        //  ESP_LOGI(TAG, "Ask to read %lu bytes", to_read);
        //  linky_rx_bytes += uart_read_bytes(UART_NUM_1, linky_buffer + linky_rx_bytes, to_read, 500 / portTICK_PERIOD_MS);
        //  ESP_LOGI(TAG, "Read %lu bytes, remaning:%ld", linky_rx_bytes, timeout - xTaskGetTickCount() * portTICK_PERIOD_MS);
        ESP_LOGI(TAG, "Read %lu bytes", linky_frame_size);
        vTaskDelay(1000 / portTICK_PERIOD_MS);
        //----------------------------------------------------------
        ...
        {
            hasFrame = false; // force timeout for standard mode (patch)
        }
    } while (!hasFrame && (MILLIS < timeout) && linky_rx_bytes < LINKY_BUFFER_SIZE - 1);
    ...
    {
        ESP_LOGE(TAG, "Error: Frame not found: Trying to decode");
        // linky_frame_size = linky_rx_bytes;
        startOfFrame = 0;
        // linky_frame_size = 0;
        // linky_frame[0] = 0;
        // return 0;
    ...
    if (linky_mode == MODE_STD)
    {
        ESP_LOGW(TAG, "keep all the buffer for standard mode: patch");
        // linky_frame_size = linky_rx_bytes;
        startOfFrame = 0;
    }
...
static char linky_decode()
{
    ...
    for (uint32_t j = 0; j < LinkyLabelListSize; j++)
    {
        ...
        void *data = LinkyLabelList[j].data;
        switch (LinkyLabelList[j].type)
        {
        case STRING:
        {
            ...
            {
                linky_decode_count++;
                // ESP_LOGI(TAG, "%s = %s", LinkyLabelList[j].label, (char *)data);
            }
            break;
        }
        ...
        case UINT8:
            ...
            {
                linky_decode_count++;
                // ESP_LOGI(TAG, "%s = %u", LinkyLabelList[j].label, *(uint8_t *)data);
            }
            break;
        case UINT16:
            ...
            {
                linky_decode_count++;
                // ESP_LOGI(TAG, "%s = %u", LinkyLabelList[j].label, *(uint16_t *)data);
            }
            break;
        case UINT32:
            ...
            {
                linky_decode_count++;
                // ESP_LOGI(TAG, "%s = %lu", LinkyLabelList[j].label, *(uint32_t *)data);
            }
            break;
        case UINT64:
            ...
            {
                linky_decode_count++;
                // ESP_LOGI(TAG, "%s = %llu", LinkyLabelList[j].label, *(uint64_t *)data);
            }
            break;
        case UINT32_TIME:
        {
            ...
            {
                linky_decode_count++;
                // ESP_LOGI(TAG, "%s = %llu %lu", LinkyLabelList[j].label, timeLabel.time, timeLabel.value);
            }
            break;
        }
        default:
