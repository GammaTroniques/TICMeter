...
// {
//     uint16_t time = *(uint16_t *)pvParameters;
//     delay(time);
//     ESP.restart();
// }
...
void pairingLEDTask(void *pvParameters)
{
    uint16_t time = *(uint16_t *)pvParameters;
    free(pvParameters);
    gpio_set_direction(PAIRING_LED_PIN, GPIO_MODE_OUTPUT);
    while (1)
    {
        gpio_set_level(PAIRING_LED_PIN, 1);
        vTaskDelay(time / portTICK_PERIOD_MS);
        gpio_set_level(PAIRING_LED_PIN, 0);
        vTaskDelay(time / portTICK_PERIOD_MS);
    }
}

// void wifiPairingTask(void *pvParameters)
// {
//     Config *config = (Config *)pvParameters;
...
//                   request->send(SPIFFS, "/reboot.html", String(), false, htmlParser);

//                   uint16_t *time = (uint16_t *)malloc(sizeof(uint16_t));
//                   *time = 2000;
//                   xTaskCreate(rebootTask, "rebootTask", 10000, time, 1, NULL);
